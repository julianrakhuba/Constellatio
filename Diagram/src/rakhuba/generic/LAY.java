...
import rakhuba.application.Indicators;
...
import rakhuba.logic.Logic;
import rakhuba.logic.SQL;
import rakhuba.logic.SearchCON;
import rakhuba.pivot.FieldMenu;
import rakhuba.pivot.LayerMenu;
import rakhuba.pivot.NSelector;
import rakhuba.pivot.PivotColumn;
import rakhuba.sidePanel.HeaderLabel;
...
import rakhuba.status.VersionType;
...
//			if(act instanceof Select && e.isControlDown()) {
//				System.out.println("populate: " + this.getAliase());
//				((Select) act).populate(this);
//			}			
//		});
...
//		runRegion.addAll(runLabel);
//		
//		runLabel.setText(getAliase());
//		runLabel.setOnMouseClicked(e ->{
////			ACT act = nnode.nmap.getNFile().getActivity();
////			if(act instanceof Select) {
////				((Select) act).populate(this);
////				((Select) act).activate(this);
////			}
//		});
		...
		layPane.setOnMouseEntered(e -> {
			...
			if(!nnode.nmap.napp.compactView.isSelected()) {
...
//		layPane.setOnContextMenuRequested(e ->{
//			if(nnode.nmap.getNFile().getActivityMode() == ActivityMode.SELECT  && this.isRoot()) {
//				nnode.nmap.getNFile().getActivity().closeActivity();
//				nnode.nmap.getNFile().setActivityMode(ActivityMode.VIEW);
//				nnode.nmap.getNFile().getActivity().passLAY(this);
...
//		});
		...
		this.setCompactView(!nnode.nmap.napp.compactView.isSelected());
...
//									|| j2.getRemoteColumn().equals(j.getColumn())
	...
	public ArrayList<String> getValuesList(String func_full_name, String full_name, String sql_name) {
		if(nnode.nmap.napp.dynamicSearch.isSelected()) {
